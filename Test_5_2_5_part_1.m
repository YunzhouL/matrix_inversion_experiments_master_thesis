...
% κ(A), κ(B) are held constant while κ(M), κ(A + BA⁻¹B) are varied,
% furthermore, κ(M), κ(A + BA⁻¹B) are significantly larger than κ(A), κ(B)
% This experimental setup isolates the effect of κ(M), κ(A + BA⁻¹B) variations on
...
for idx = 1:length(matrix_sizes)
    ...
    for trial = 1:num_trials
        rng(trial);  % 固定随机种子确保可重复性

        % Generate M = A +iB with κ(A) = κ(B) = 1
        % κ(A + BA⁻¹B) = 1000, κ(M) = 2000 or
        % κ(A + BA⁻¹B) = 100000, κ(M) = 200000
        A = eye(n);
        % κ(A + BA⁻¹B) = 1000, κ(M) = 2000
        [B,D,U] = generate_random_bad_orthogonal_matrix(n, 0.001);
        % κ(A + BA⁻¹B) = 100000, κ(M) = 200000
        % [B,D,U] = generate_random_bad_orthogonal_matrix(n, 0.00001);
        s = 1 ./ diag(A + 1i * D);
        S_inv = diag(s);
...
title('Accuracy Comparison ( Delta = 0.001 )', 'FontSize', 18);
set(gca, 'FontSize', 14);
xlim([100 2000]);
grid on;
...
% filename = fullfile(pwd, 'Fig_5_2_5_part_1_1.png');
% exportgraphics(fig, filename, 'Resolution', 300);

% filename = fullfile(pwd, 'Fig_5_2_5_part_1_2.png');
% exportgraphics(fig, filename, 'Resolution', 300);