...
% The comparison is performed on random unitary matrices to evaluate
...
for idx = 1:length(matrix_sizes)
    ...
    for trial = 1:num_trials
        ...
        M = RandUniMat(n); % Generate random unitary matrix
       
        % Algo-LU
        X1 = inverse_via_LU(M);
        errors(trial, 1) = norm(X1 - M','fro') / sqrt(n);

        % Algo-Frob
        X2 = inverse_Frobenius(M);
        errors(trial, 2) = norm(X2 - M','fro') / sqrt(n);

        % Algo-QR
        X3 = inverse_via_QR(M);
        errors(trial, 3) = norm(X3 - M', 'fro') / sqrt(n);

        % Algo-SVD
        X4 = inverse_via_SVD(M);
        errors(trial, 4) = norm(X4 - M', 'fro') / sqrt(n);
    end
...
% filename = fullfile(pwd, 'Fig_5_2_1_part_3.png');
