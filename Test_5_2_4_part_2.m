...
% κ(M), κ(A), κ(A + BA⁻¹B) are held constant while  κ(B) is varied
% This experimental setup isolates the effect of κ(B) variations on
...
for idx = 1:length(matrix_sizes)
    ...
    for trial = 1:num_trials
        ...
        % Generate M = A +iB with κ(M) = 28, κ(A) = 200, κ(A + BA⁻¹B) = 4,
        % κ(B) = 200 or 20
        while cond(M) <= 20 || cond(M) >= 2000
            U1 = RandOrthMat(n);
            V1 = RandOrthMat(n);
            % κ(B) = 200
            [S1, S2] = generate_big_B(n, 200);
            % κ(B) = 20
            % [S1, S2] = generate_small_B(n, 500);
            s1 = 1 ./ diag(S1);
            S1_inv = diag(s1);
            A = U1 * S1 * V1';
            B = U1 * S2 * V1';
...
% title('Accuracy Comparison ( k(B) = 200 )', 'FontSize', 18);
title('Accuracy Comparison ( k(B) = 20 )', 'FontSize', 18);
set(gca, 'FontSize', 14);
% xlim([100 2000]);
% ylim([0 1.5e-13]);
grid on;
...
% filename = fullfile(pwd, 'Fig_5_2_4_part_2_1.png');
% exportgraphics(fig, filename, 'Resolution', 300);

% filename = fullfile(pwd, 'Fig_5_2_4_part_2_2.png');
% exportgraphics(fig, filename, 'Resolution', 300);