...
% κ(M) = κ(A) = κ(B) are held constant while κ(A + BA⁻¹B) is varied
% This experimental setup isolates the effect of κ(A + BA⁻¹B) variations on
...
for idx = 1:length(matrix_sizes)
    ...
    for trial = 1:num_trials
        ...
        % Generate M = A +iB with κ(M) = κ(A) = κ(B) = 200,
        % κ(A + BA⁻¹B) = 200 or 20000
        while cond(M) <= 20 || cond(M) >= 2000
            U1 = RandOrthMat(n);
            V1 = RandOrthMat(n);
            % κ(A + BA⁻¹B) = 200
            [S1, S2] = generate_two_random_diag_kappa_with_relation(n, 200);
            % κ(A + BA⁻¹B) = 20000
            % [S1, S2] = generate_comp_greater(n, 200);
            s1 = 1 ./ diag(S1);
            S1_inv = diag(s1);
            A = U1 * S1 * V1';
            B = U1 * S2 * V1';
...
title('Accuracy Comparison ( k(A+BA^{-1}B) = k(M) )', 'FontSize', 18);
% title('Accuracy Comparison ( k(A+BA^{-1}B) > k(M) )', 'FontSize', 18);
set(gca, 'FontSize', 14);
xlim([100 2000]);
ylim([0 2e-13]);
...
% filename = fullfile(pwd, 'Fig_5_2_3_1.png');
% exportgraphics(fig, filename, 'Resolution', 300); 

% filename = fullfile(pwd, 'Fig_5_2_3_2.png');
% exportgraphics(fig, filename, 'Resolution', 300); 